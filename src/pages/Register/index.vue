...
<!--      </div>-->
      ...
      <Form ref="target" :validation-schema="validateRules" v-slot="{ errors }">
        <div class="content">
          <label>手机号:</label>
          <Field
              placeholder="请输入你的手机号"
        ...
        <div class="content">
          ...
          <Field
              ...
              name="password1"
          />
          <span class="error-msg" v-if="errors.password1">{{ errors.password1 }}</span>
        </div>
        <div class="controls">
          <Field
              type="checkbox"
...
export default {
  ...
  setup(){
    ...
    const validateRules = {
      ...
      password1: validateForm.password1,
      agree: validateForm.agree
    }
    return {
      formMess,
      validateRules
  ...
  methods: {
    ...
    //用户注册
    async userRegister() {
      const success = await this.$refs.target.validate();
